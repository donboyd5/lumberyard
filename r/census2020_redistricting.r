...
# based on pl_all_4_2020_dar.r
# data at: https://www2.census.gov/programs-surveys/decennial/2020/data/01-Redistricting_File--PL_94-171/
...
zpath <- file.path(dpl, "ny2020.pl.zip")
archive(zpath)

header_fname <- "nygeo2020.pl"
part1_fname <- "ny000012020.pl"
part2_fname <- "ny000022020.pl"
part3_fname <- "ny000032020.pl"
...
# must re-run archive_read before each read
header <- vroom(archive_read(zpath, header_fname), delim="|", col_names = FALSE, col_types = cols(.default="c"))
part1 <- vroom(archive_read(zpath, part1_fname), delim="|", col_names = FALSE, col_types = cols(.default="c"))
part2 <- vroom(archive_read(zpath, part2_fname), delim="|", col_names = FALSE, col_types = cols(.default="c"))
part3 <- vroom(archive_read(zpath, part3_fname), delim="|", col_names = FALSE, col_types = cols(.default="c"))
...
# combine2 <- header[,-7] part1[,-4], part2[,-4], part3
...
combine2 <- combine %>%
  ...
  mutate(across(c(arealand, areawatr, pop100, hu100, intptlat, intptlon,
                  starts_with("p00"),
                  starts_with("h00")), as.numeric))
glimpse(combine2)
saveRDS(combine2, file.path(dpl, "census2020plny.rds"))

combine2 %>%
  filter(str_detect(basename, "Cambridge"),
         !str_detect(name, "part")) %>%
  select(sumlev, geoid, county, basename, name, arealand, areawatr, pop100, hu100, intptlat, intptlon) %>%
  arrange(name)
