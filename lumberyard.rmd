...
glimpse(df)
tab_B01002 <- df %>%
  pivot_longer(starts_with("x"), names_to = "vname", values_to = "mdnage") %>%
  filter(!is.na(mdnage)) %>%
  mutate(vname=case_when(vname %in% c("x156", "x94") ~ "total",
                         vname %in% c("x157", "x95") ~ "male",
                         vname %in% c("x158", "x96") ~ "female")) %>%
  select(year, table, vname, valtype, sgeotype, geoid, usgeoname, mdnage) 


tabdata <- tab_B01002 %>%
  filter(eval(key_subareas)) %>% 
  filter(!str_detect(usgeoname, "Schuy"),
         !str_detect_any(usgeoname, c("Dresden", "Hampton", "Putnam"))) %>%
  filter(vname=="total", valtype=="est") %>%
  select(year, table, geoid, sgeotype, usgeoname, mdnage) %>%
  pivot_wider(names_from = year, names_prefix = "y", values_from = mdnage) %>%
...
tab <- tabdata %>%
  ...
  tab_header(
    title = "Median age in selected areas"
      ) %>%
  cols_label(usgeoname="",
             y2009="2009",
             y2014="2014",
