...
## Population size
```{r}
# get the fewest-variables table for total population
# B01003	Total population

df <- map_dfr(c(2009, 2014, 2019), get_yeartab, "B01003")
names(df)
df2 <- df %>%
  mutate(pop=ifelse(is.na(x186), x124, x186)) %>%
  select(year, table, valtype, sgeotype, geoid, usgeoname, pop) %>%
  pivot_wider(names_from = c(valtype, year), values_from = pop)
df2

tabdata <- df2 %>%
  filter((sgeotype=="state" & geoid %in% c("04000US36", "04043US36")) |
           (sgeotype=="county" & 
              str_detect_any(usgeoname, c("Saratoga", "Warren", "Washington"))) |
           (sgeotype=="cosub" & str_sub(geoid, 8, 12)=="36115") |  # towns
           (sgeotype %in% c("place"))) %>% # villages
  filter(!str_detect(usgeoname, "Schuy"),
         !str_detect_any(usgeoname, c("Dresden", "Hampton", "Putnam"))) %>%
  select(table, geoid, sgeotype, usgeoname, starts_with("est")) %>%
  mutate(pch0914=est_2014 / est_2009 - 1,
         pch1419=est_2019 / est_2014 - 1,
         rowlab=factor(sgeotype,
...
tab <- tabdata %>%
  ...
  cols_hide(c(table, geoid, sgeotype)) %>%
  tab_header(
    title = "Population in selected areas"
      ) %>%
  cols_label(usgeoname="",
             est_2009="2009",
             est_2014="2014",
             est_2019="2019",
             pch0914="2009 to 2014",
             pch1419="2014 to 2019",
             ) %>%
  tab_spanner(
...
gtsave(tab, filename="tab_poptot.png", path=here::here("results"))
